...
const CameraComponent: React.FC<CameraComponentProps> = (props) => {
  ...
  const [cameraWidth, setCameraWidth] = useState<number>(299);
 ...
 // if (device == null) return <NoCameraErrorView />
  ...
  return (
    <View style={GetCameraComponentStyles(Android_Theme_Light).container}>


      <View style={[
        (props.cameraType === 'front' ? GetCameraComponentStyles(Android_Theme_Light).frontCameraView : GetCameraComponentStyles(Android_Theme_Light).backCameraView),
        { width: props.cameraType === 'front' ? cameraWidth : (cameraWidth + 120) }
      ]}>
        ...
        {!isActive && (
          imageSource !== "" ? (
            <Image style={{ width: "100%", height: "100%", transform: [{ rotate: props.cameraType === 'front' ? '90deg' : '0deg' }] }} source={{ uri: `file://${imageSource}` }} />
          ) : null
        )}
      </View>
      {/* <TouchableOpacity onPress={props.onClose} style={GetCameraComponentStyles(Android_Theme_Light).closeButtonContainer}>
        <Text style={GetCameraComponentStyles(Android_Theme_Light).closeButton}>Close</Text>
      </TouchableOpacity> */}
      <View style={GetCameraComponentStyles(Android_Theme_Light).captureButtonContainer}>
        {imageSource !== "" ? (
          <View style={GetCameraComponentStyles(Android_Theme_Light).buttonsContainer}>
            <TouchableOpacity onPress={retryCapture} style={GetCameraComponentStyles(Android_Theme_Light).captureButton}>
              <Text style={GetCameraComponentStyles(Android_Theme_Light).captureButtonText}>Retry</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={SavePhoto} style={GetCameraComponentStyles(Android_Theme_Light).captureButton}>
          ...
          <TouchableOpacity onPress={() => capturePhoto()} style={GetCameraComponentStyles(Android_Theme_Light).captureButton}>
            <Text style={GetCameraComponentStyles(Android_Theme_Light).captureButtonText}>CAPTURE</Text>
          </TouchableOpacity>
        )}
      </View>
